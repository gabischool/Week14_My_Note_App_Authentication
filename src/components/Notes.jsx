...
function Notes() {
  ...
  return (
    <div className="flex flex-wrap justify-center">
      {notes.map((note) =>  (
        <div
          className="relative bg-white w-64 h-64 m-5 border-t-[4px] border-[#00BEFE] shadow-2xl rounded overflow-hidden transition-all duration-100 hover:scale-105"
          key={note.id}
        >
          <div className="p-5">
          ...
          <Link to={`/edit/${note.id}`}>
            <button className="mr-2 text-[#00BEFE]">
              <FaEdit size={20} />
            </button>
            </Link>
